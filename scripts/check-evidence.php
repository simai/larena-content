...
if (!is_array($context) || ($context['evidence_path'] ?? null) !== EVIDENCE_PATH) {
    $errors[] = 'Launch context does not point to the CMS content model v1 evidence bundle.';
}

foreach (['implementation-summary.md', 'tests.md', 'smoke.md', 'file-map.json', 'deviations.json', 'graph-sync-proposal.json'] as $file) {
...
fwrite(STDOUT, 'CMS content model v1 evidence bundle is structurally complete.'.PHP_EOL);
