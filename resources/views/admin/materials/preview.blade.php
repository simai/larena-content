...
@section('content')<section class="larena-admin-card"><dl>@foreach($values as $key => $value)<dt><strong>{{ $key }}</strong></dt><dd>@if(is_bool($value)){{ $value ? __('larena-content::admin.boolean.yes') : __('larena-content::admin.boolean.no') }}@else{{ $value }}@endif</dd>@endforeach</dl></section>@endsection
