...
@if(!$type)
<section class="larena-panel larena-home-panel"><h2>{{ __('larena-content::admin.materials.choose_type') }}</h2>@if($types === []){!! $ui->alert(__('larena-content::admin.materials.no_types'), 'warning') !!}@else<form method="get" action="{{ route('larena.content.admin.materials.create') }}" class="larena-form"><label class="larena-field">{{ __('larena-content::admin.fields.type') }}<select name="type_key" required>@foreach($types as $option)<option value="{{ $option['key'] }}">{{ $option['label'] }}</option>@endforeach</select></label><div class="larena-form-actions">{!! $ui->button('actions.continue', 'primary') !!}</div></form>@endif</section>
@else
<form method="post" action="{{ $material ? route('larena.content.admin.materials.update', $material->itemRef->uuid()) : route('larena.content.admin.materials.store') }}">@csrf @if($material)@method('put')<input type="hidden" name="expected_revision" value="{{ $material->currentRevision }}">@else<input type="hidden" name="type_key" value="{{ $type->typeKey->value }}">@endif
<section class="larena-panel larena-form"><div class="larena-form-grid">
@if(!$material)<label class="larena-field">{{ __('larena-content::admin.fields.locale') }}<input name="locale" value="{{ old('locale', 'en') }}" required></label>@endif
<label class="larena-field">{{ __('larena-content::admin.fields.slug') }}<input name="slug" value="{{ old('slug', $material?->currentSlug->value) }}" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" aria-describedby="larena-slug-help" @disabled($material && !$canUpdate)><small id="larena-slug-help">{{ __('larena-content::admin.materials.slug_help') }}</small></label>
<label class="larena-field">{{ __('larena-content::admin.fields.visibility') }}<select name="visibility" @disabled($material && !$canUpdate)><option value="public" @selected(old('visibility', $material?->currentVisibility->value ?? 'public') === 'public')>{{ __('larena-content::admin.visibility.public') }}</option><option value="private" @selected(old('visibility', $material?->currentVisibility->value ?? 'public') === 'private')>{{ __('larena-content::admin.visibility.private') }}</option></select></label>
</div></section>
@if($material && !$canUpdate){!! $ui->alert(__('larena-content::admin.materials.read_only'), 'info') !!}@endif
<section class="larena-panel larena-form"><h2>{{ __('larena-content::admin.materials.fields_heading') }}</h2><div class="larena-form-grid">
@foreach($type->fieldDefinitions as $field)
<label class="larena-field">{{ $ui->fieldLabel($field->key) }} <small>{{ __('larena-content::admin.field_types.'.$field->propertyType) }}</small>
...
@if(!$material || $canUpdate)<div class="larena-form-actions">{!! $ui->button($material ? 'actions.save_material' : 'actions.create_material', 'primary') !!}</div>@endif</form>

@if($material)<section class="larena-panel larena-home-panel"><h2>{{ __('larena-content::admin.workflow.heading') }}</h2><p>{{ __('larena-content::admin.materials.current_state', ['status' => $material->currentStatus->value, 'revision' => $material->currentRevision]) }}</p><div class="larena-form-actions">
