...
<label>{{ __('larena-content::admin.fields.slug') }}<input name="slug" value="{{ old('slug', $material?->currentSlug->value) }}" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*"></label>
<label>{{ __('larena-content::admin.fields.visibility') }}<select name="visibility"><option value="public" @selected(old('visibility', $material?->currentVisibility->value ?? 'public') === 'public')>{{ __('larena-content::admin.visibility.public') }}</option><option value="private" @selected(old('visibility', $material?->currentVisibility->value ?? 'public') === 'private')>{{ __('larena-content::admin.visibility.private') }}</option></select></label>
</div></section>
...
<label>{{ $field->key }} <small>{{ __('larena-content::admin.field_types.'.$field->propertyType) }}</small>
@if($field->propertyType === 'text')<textarea name="values[{{ $field->key }}]" @required($field->required)>{{ old('values.'.$field->key, $values[$field->key] ?? '') }}</textarea>
@elseif($field->propertyType === 'boolean')<input type="hidden" name="values[{{ $field->key }}]" value="0"><input type="checkbox" name="values[{{ $field->key }}]" value="1" @checked(old('values.'.$field->key, $values[$field->key] ?? false))>
@elseif($field->propertyType === 'file')<select name="values[{{ $field->key }}]" @required($field->required)><option value="">{{ __('larena-content::admin.materials.no_file') }}</option>@foreach($files as $file)<option value="{{ $file['ref'] }}" @selected(old('values.'.$field->key, $values[$field->key] ?? '') === $file['ref'])>{{ $file['label'] }}</option>@endforeach</select>
@elseif($field->propertyType === 'relation')<select name="values[{{ $field->key }}]" @required($field->required)><option value="">{{ __('larena-content::admin.materials.no_relation') }}</option>@foreach($relations as $relation)<option value="{{ $relation['ref'] }}" @selected(old('values.'.$field->key, $values[$field->key] ?? '') === $relation['ref'])>{{ $relation['label'] }}</option>@endforeach</select>
@else<input type="{{ $field->propertyType === 'date' ? 'date' : ($field->propertyType === 'number' ? 'number' : 'text') }}" @if($field->propertyType === 'number')step="any"@endif name="values[{{ $field->key }}]" value="{{ old('values.'.$field->key, $values[$field->key] ?? '') }}" @required($field->required)>@endif
</label>@endforeach
</div></section>
